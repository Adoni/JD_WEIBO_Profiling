...
def construct_knn():
    ...
    for index,user in enumerate(users.find()):
        ...
        items=reduce(lambda x,y:x+y,map(lambda behavior:behavior['parsed_review']['review_general'],user['behaviors']),[])
        shopping_record[uid]=set(items)
        #shopping_record[uid]=items
        bar.draw(index+1)

    finish_count=0
    bar=progress_bar(len(shopping_record))
    #fout=open(RAW_DATA_DIR+'knn_graph_from_shopping_record.data','w')
    fout=open(RAW_DATA_DIR+'knn_graph_from_review.data','w')
    ...
    for uid1 in shopping_record:
        ...
        for uid2 in shopping_record:
            ...
            #a=shopping_record[uid1]
            #b=shopping_record[uid2]
            #n=reduce(lambda x,y:x+int(y in b),a,0)
            #n=len(filter(lambda x:x in b,a))
            if n==0:
                continue
